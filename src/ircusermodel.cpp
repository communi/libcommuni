...
protected:
    ...
    {
        const QString p1 = sourceModel()->data(left, Irc::PrefixRole).toString();
        const QString p2 = sourceModel()->data(right, Irc::PrefixRole).toString();

        const int i1 = !p1.isEmpty() ? pfx.indexOf(p1.at(0)) : -1;
        const int i2 = !p2.isEmpty() ? pfx.indexOf(p2.at(0)) : -1;
        ...
        const QString n1 = sourceModel()->data(left, Irc::NameRole).toString();
        const QString n2 = sourceModel()->data(right, Irc::NameRole).toString();
...
void IrcUserModelPrivate::_irc_processMessage(IrcMessage* message)
{
    ...
    if (!info.isValid()) {
        ...
        if (proxy)
            proxy->sortByPrefixes(info.prefixes());
    }
...
/*!
    Returns a sorted proxy model. The users are sorted by mode and alphabetically.

    \note The proxy model is lazily created if it does not exist.
 */
QSortFilterProxyModel* IrcUserModel::proxy() const
{
    Q_D(const IrcUserModel);
    if (!d->proxy) {
        IrcUserModel* that = const_cast<IrcUserModel*>(this);
        that->d_func()->proxy = new IrcUserProxyModel(that);
        if (d->info.isValid())
            d->proxy->sortByPrefixes(d->info.prefixes());
    }
    return d->proxy;
}
